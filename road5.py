...
class AGVCommunication:
    ...
    # 修改parse_task_data函数，将二维码数字转换为矩阵坐标
    def parse_task_data(self, data):
        """解析从控制端接收的任务数据（二维码数字转换为坐标）"""
        # 协议格式：任务类型(1B) + 初始位置二维码数字(2B) + 初始方向(1B) + 
        #           取货位置二维码数字(2B) + 空闲列二维码数字(2B)
        if len(data) < 8:
            print(f"数据长度不足: {len(data)}字节")
            return None
        
        task_type = data[0]
        init_qr_value = (data[1] << 8) | data[2]  # 初始位置二维码数字
        init_direction_val = data[3]
        pick_qr_value = (data[4] << 8) | data[5]  # 取货位置二维码数字
        empty_qr_value = (data[6] << 8) | data[7]  # 空闲列二维码数字
        ...
        # 将二维码数字转换为矩阵坐标
        def qr_value_to_coord(qr_value):
            """将二维码数字转换为矩阵坐标(row, col)"""
            if excel_matrix is not None:
                # 在Excel矩阵中查找对应的坐标
                for row in range(excel_matrix.shape[0]):
                    for col in range(excel_matrix.shape[1]):
                        if not np.isnan(excel_matrix[row, col]) and int(excel_matrix[row, col]) == qr_value:
                            return (row, col)
            print(f"警告: 未找到二维码数字 {qr_value} 对应的坐标")
            return (0, 0)  # 默认值
        
        init_pos = qr_value_to_coord(init_qr_value)
        pick_pos = qr_value_to_coord(pick_qr_value)
        empty_col = qr_value_to_coord(empty_qr_value)
        ...
        task_info = {
            ...
            'pick_pos': pick_pos,
            'empty_col': empty_col,
            'qr_values': {  # 保存原始二维码数字，用于后续编码
                'init': init_qr_value,
                'pick': pick_qr_value,
                'empty': empty_qr_value
            }
        }
        
        print(f"解析到任务数据: 类型={task_type}, 初始位置二维码={init_qr_value}->坐标{init_pos}, "
            f"初始方向={DIR_NAMES[init_direction]}, 取货位置二维码={pick_qr_value}->坐标{pick_pos}, "
            f"空闲列二维码={empty_qr_value}->坐标{empty_col}")
        
        return task_info
    
    # 修改encode_path_data函数，使用二维码数字而不是坐标
    def encode_path_data(self, detailed_path):
        """将详细路径编码为传输格式（使用二维码数字），并包含动作和速度信息"""
        ...
        # 构建路径点数据（每个点包含二维码数字、动作代码、速度值）
        path_data = bytearray()
        for step in detailed_path:
            # 获取二维码数字
            qr_value = step.get('excel_value', 0)
            
            # 如果excel_value是字符串格式的坐标，尝试转换为数字
            if isinstance(qr_value, str) and qr_value.startswith('('):
                try:
                    row, col = map(int, qr_value.strip('()').split(','))
                    # 从Excel矩阵中获取二维码数字
                    if (0 <= row < excel_matrix.shape[0] and 
                        0 <= col < excel_matrix.shape[1] and
                        not np.isnan(excel_matrix[row, col])):
                        qr_value = int(excel_matrix[row, col])
                    else:
                        qr_value = 0
                except:
                    qr_value = 0
            ...
            # 将二维码数字分解为两个字节
            qr_high = (qr_value >> 8) & 0xFF
            qr_low = qr_value & 0xFF
            
            # 获取动作代码
            action_code = action_map.get(step['action'], 0x00)
            
            # 获取速度值（乘以10转换为整数，保留一位小数精度）
            speed_int = int(step['speed'] * 10) & 0xFF  # 限制在0-255范围内
            
            # 添加路径点数据：二维码高字节、二维码低字节、动作代码、速度值
            path_data.extend([qr_high, qr_low, action_code, speed_int])
            
            print(f"编码路径点: 二维码={qr_value}(0x{qr_high:02X}{qr_low:02X}), "
                f"动作={step['action']}(0x{action_code:02X}), "
                f"速度={step['speed']}m/s(0x{speed_int:02X})")
        ...
        # 打印调试信息
        print(f"完整数据帧: {frame_header.hex(' ').upper()} {final_data.hex(' ').upper()}")
        print(f"总长度: {len(frame_header) + len(final_data)}字节")
...
# 修改task_callback函数，确保路径中包含二维码数字
def task_callback(task_data, agv_instance):
    ...
    print(f"起始位置二维码: {task_data['qr_values']['init']} -> 坐标{task_data['init_pos']}")
    print(f"起始方向: {DIR_NAMES[task_data['init_direction']]}")
    print(f"取货位置二维码: {task_data['qr_values']['pick']} -> 坐标{task_data['pick_pos']}")
    print(f"空闲列二维码: {task_data['qr_values']['empty']} -> 坐标{task_data['empty_col']}")
    print("=" * 50)
    ...
    if detailed_path:
        ...
        for step in detailed_path:
            ...
            if (0 <= row < excel_matrix.shape[0] and 
                ...
                converted_step['excel_value'] = int(excel_matrix[row, col])
                converted_path.append(converted_step)
            else:
                # 如果坐标超出范围或为NaN，使用坐标字符串
                converted_step = step.copy()
                converted_step['excel_value'] = f"({row},{col})"
                converted_path.append(converted_step)
                print(f"警告: 坐标({row},{col})没有对应的二维码数字")
        
        print("\n详细路径规划结果（使用二维码数字）:")
        for i, step in enumerate(converted_path):
            print(f"步骤 {i+1}: 二维码{step['excel_value']}, 方向:{step['direction_name']}, "
                  f"动作:{step['action']}, 速度:{step['speed']}m/s, 区域:{step['speed_zone']}")
        
        # 发送路径数据回控制端（使用二维码数字）
        if agv_instance.send_path(converted_path):
            print("路径数据已成功发送到控制端")
        else:
            print("路径数据发送失败")
    else:
...
def main():
    ...
    # 加载Excel表格
    excel_file_path = r"E:\work\Mushroom\Project_luronggu\road_choose\e_area.xlsx"  # 替换为您的Excel文件路径
    excel_matrix = load_excel_matrix(excel_file_path)
    
    if excel_matrix is None:
        print("无法加载Excel表格，程序退出")
        return
    
    print(f"成功加载Excel表格，形状: {excel_matrix.shape}")
