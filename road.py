...
class AGVCommunication:
                        ...
                        if buffer[0] == 0xAB and buffer[1] == 0xBA:
                            ...
                            if len(buffer) >= 5:  # 至少要有命令+长度
                                ...
                                if len(buffer) >= total_length:
                                    ...
                                    crc_calculated = self.CRC8(frame[2:-1])  # 从命令到数据末尾
    ...
    def get_speed_zone(self, row, col):
        """获取速度区域（示例实现，需要根据你的实际地图调整）"""
        if row == 2:
            return "main"
        elif 5 <= row <= 10 and 2 <= col <= 15:
            return "shelf"
...
# ==================== 使用示例 ====================
def example_task_callback(task_data, agv_instance):
    """任务回调函数示例"""
    print(f"收到任务数据: {task_data}")
    ...
    # 示例：生成一个假的路径用于测试
    # detailed_path = [
    #     {'position': (0, 6), 'action': 'forward', 'speed': 2.0},
    #     {'position': (1, 6), 'action': 'forward', 'speed': 2.0},
    #     {'position': (2, 6), 'action': 'right_turn', 'speed': 1.0},
    #     {'position': (2, 7), 'action': 'forward', 'speed': 2.0},
    #     {'position': (2, 7), 'action': 'stop', 'speed': 0.0}
    # ]

    # 示例使用 - 初始车头朝下
    order_command = 1 # 任务类别：出库、入库
    task_type = 1 # 默认任务为入库
    if order_command == 2:
    ...
    init_direction = DOWN
    pick_pos = (7, 6) # 目标取货位置
    empty_col = (7, 12) # 取货区空闲列

    if task_type == 1:
        print("开始规划入库路径...")
    else:
        print("开始规划出库路径...")
    detailed_path = plan_restock_task(init_pos, init_direction, pick_pos, empty_col, task_type)
    ...
    # 发送路径
    agv_instance.send_path(detailed_path)
...
def main():
    ...
    if available_ports:
        ...
        # 使用lambda函数将agv_com传递给回调
        callback_with_agv = lambda task_data: example_task_callback(task_data, agv_com)

        # 启动通信
        if agv_com.start(callback_with_agv):
            try:
                print("按Ctrl+C退出程序...")
                while True:
                    time.sleep(1)
